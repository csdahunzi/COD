...
def p2t_small(pretrained=True, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model_state = torch.load('../models/p2t_small.pth')
...
def p2t_base(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model_state = torch.load('/cver/dcjxc/BGNet/models/p2t_base.pth')
        model.load_state_dict(model_state)
    return model
...
def p2t_medium(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model_state = torch.load('../models/p2t_medium.pth')
        model.load_state_dict(model_state)
...
def p2t_large(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        model_state = torch.load('/cver/dcjxc/BGNet/models/p2t_large.pth')
        model.load_state_dict(model_state)
    return model
